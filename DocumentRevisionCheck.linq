...
// -- configuration ---------------
// Hier den Statuswert eintragen, den ein neues Dokument am Anfang bekommt
// Enter the status value that a new document receives at the beginning here
string wipState = "00001";
// Hier die Revision eintragen, die ein neues Dokument bekommt. Im PSP Standard wird keine Revision gesetzt
...
HashSet<string> releasedStates = new HashSet<string> {"00003"};
HashSet<string> obsoleteStates = new HashSet<string> {"00005"};
HashSet<string> quickFixStates = new HashSet<string> {"00006"};
// Die folgenden Schalter werden benötigt um detaillierte Ausgaben zu bekommen.
// Am besten Schalter so eingestellt lassen. Wenn man einzelne Dokument prüft ist es sinnvoll die "debug" auf "true" zu setzen
...
void Main()
{
	var docs = (from e in VIEW_ALL_DOCUMENT //where e.IDENT == "ENG-000016"
		orderby e.REVISION
		orderby e.IDENT
		select new { e.IDENT, e.REVISION, e.AIMKEY, e.STATUSKEY, e.CREATE_DATE, e.CHANGE_DATE, e.CHECKED_1_DATE }); //.Take(10);
	...
	foreach (var d in docs)
	{
		...
		var historyData = from h in VIEW_HISTORY_DOCUMENT where h.AIMKEY == d.AIMKEY select new { h.IDENT, h.REVISION, h.CHECK_DATE, h.CHANGE_DATE };
		...
		if (currentIdent != d.IDENT)
		{
			...
			AddInitialHistoryFile(d.STATUSKEY, d.REVISION, d.CREATE_DATE, d.CHECKED_1_DATE, (historyData.Count() > 0));
		}
		...
		if (historyData.Count() == 0)
		{
			...
			else
			{
				if (quickFixStates.Contains(d.STATUSKEY))
					AddAndCheckDate(currentDate, d.REVISION, d.STATUSKEY, "predecessor date"); // in case of quickFix files use date of predecessor
...
public void AddInitialHistoryFile(string statuskey, string revision, object createDate, object checkDate, bool isHistoryData)
{
	if (!isHistoryData)
	{	
